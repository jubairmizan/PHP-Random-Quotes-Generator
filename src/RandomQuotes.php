...
class RandomQuotes{
	public function getRandomWord($len = 10) {
	    ...
	    // return array_slice($splitWord,0,$len);

	    $randomWord = '';
	    foreach(array_rand($splitWord,$len) as $value){
	    	$randomWord .= $splitWord[$value].' ';
	    }

	    return $randomWord;
	}
	public function generateRandomQuotes($len = 10){
		return $this->getRandomWord($len);
	}
}